...
export default {
  ...
  data: () => ({
    ...
    idRules: [
      (v) => !!v || "El campo es requerido",
      (v) =>
        (v && (v.length === 10 || v.length === 8)) ||
        "Ingrese un numero de identidad correcto",
    ],
    ageRules: [
      (v) => !!v || "El campo es requerido",
    ...
    cellphoneNumber: [
      ...
      (v) =>
        (v && v.length == 10) ||
        "Su numero de celular debe ser de 10 carácteres, omita el '+57'",
    ],
    address: [
    ...
    passwordRules: [
      ...
      (v) =>
        (v && v.length <= 15) ||
        "La contraseña puede ser de maximo 15 carácteres",
      (v) =>
        (v && v.length >= 8) ||
        "La contraseña tiene que tener minimo 8 carácteres",
    ],
    email: "",
    emailRules: [
  ...
  methods: {
    ...
    editUser() {
      if (this.$refs.formEdit.validate() && this.formEdit) {
        const url =
          "http://localhost:3001/api/v2/users/" + this.user.cedulausuario;
        let data = {};
